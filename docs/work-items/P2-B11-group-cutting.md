...
> 상태: `READY` — 2026-09-12 사용자 결정 `A~D` 완료. 착수 조건: MFC 실제 DXF 파일 확보(`D2-B11-B`)와 상세 설계
>
...
### `D2-B11-A` 이 기능을 실제로 쓰는 고객이 있는가 — **확정: (가) 진행. 그룹재단이 주력이다**
...
**2026-09-12 사용자 결정: 그룹재단이 메인이다.** 부가 기능이 아니라 주력 기능으로 다룬다. 이 결정이 `D`의 순서를 바꿨다.

### `D2-B11-B` 장비용 DXF 의 레이어·파일명 규칙을 MFC 그대로 따를 것인가 — **확정: (가) MFC 규칙 그대로**
...
**2026-09-12 사용자가 (가)를 확정했다.** 주력 기능일수록 전환 시점의 변수를 줄인다.

착수 조건: **MFC 로 낸 실제 DXF 파일 한 벌**(원판 파일 + 그룹 파일)이 필요하다. 소스만으로는 좌표 원점·회전(`nBaseDraw` 0~3)·오프셋을 확신할 수 없다. 레거시 DB 에는 없으므로 현장 PC 의 `export_dxf` 폴더에서 받아야 한다. 없으면 B11-1 을 시작할 수는 있어도 합격 판정을 못 한다.

### `D2-B11-C` 그룹 지정을 어느 개정에 붙이는가 — **확정: (가) 재단 개정에 붙이고 함께 잠근다**

| 안 | 결과 |
|---|---|
| **(가) 재단 개정에 붙이고 개정과 함께 잠근다** (권장) | 승인된 재단 결과와 DXF 가 1:1 로 남는다. 재실행하면 지정은 새 개정으로 옮겨 오지 않는다(부품 배치가 바뀌므로) |
| (나) 재단 작업 단위로 두고 개정과 무관하게 유지 | 재실행 뒤에도 지정이 남지만 배치가 바뀐 뒤엔 사각형이 어긋나 검증에서 대부분 걸린다. 걸리지 않고 살아남은 지정이 오히려 위험하다 |

**2026-09-12 사용자가 (가)를 확정했다.** 웹 재단 편집은 고정 후 재실행(`D2-B05-J`)이라 배치가 바뀌는 것이 정상 흐름이고, MFC 도 지정을 저장하지 않고 그 자리에서 DXF 를 냈다. 재실행 뒤 다시 지정하는 수고는 나중에 "이전 개정 지정 가져오기(검증 통과분만)" 로 줄일 수 있으나 이번 범위에 넣지 않는다.

### `D2-B11-D` 순서 — `P2-B07`~`B10` 앞인가 뒤인가 — **확정: (가) `P2-B11` 을 출력 묶음보다 먼저**

| 안 | 결과 |
|---|---|
| **(가) `P2-B11` 을 출력 묶음보다 먼저** | B11-1 → B11-2 → B11-3 → B11-4, 그 다음 B07~B10. B11-1 재단 DXF 가 B09 대량 DXF 의 단건 기반이 된다 |
| (나) 원래 순서대로 B07~B10 먼저 | 주력 기능이 가장 뒤로 간다. B07 실물 확인이 늦어지면 같이 밀린다 |
| (다) B11-1 만 B09 에 합치고 나머지는 뒤에 | 그룹재단이 부가 기능일 때 맞는 안 |

**2026-09-12 사용자가 (가)를 확정했다.** `D2-B11-A` 에서 주력으로 확인됐고, B07 은 용지·프린터 기준 미결정으로 막혀 있어 그 사이에 세운다. [P2 실행계획](./P2-execution-plan.md)의 순서를 이에 맞춰 고친다.

## 5. 착수 전 남은 것

| 항목 | 내용 |
|---|---|
| MFC 실제 DXF 파일 | 현장 PC `export_dxf` 폴더에서 원판 파일과 그룹 파일 한 벌. 합격 기준의 대조 대상 |
| 상세 설계 | annotations 스키마(`laserGroups`·`horizontalCutLines`·`sheets[].film`), 검증 규칙(같은 종류·인접 사각형), API, 화면 선택 모드. B11-1 착수 전에 이 문서 3절을 확정안으로 다시 쓴다 |
...
| 날짜 | 변경 내용 | 작성자 |
|---|---|---|
| 2026-09-12 | MFC 소스 확인 결과로 검토 문서 최초 작성. 그룹재단 = 레이저 그룹 지정 + 장비용 DXF | Claude |
| 2026-09-12 | `D2-B11-A~D` 사용자 결정. 그룹재단이 주력(A), MFC DXF 규칙 그대로(B), 지정은 재단 개정에 붙여 잠금(C), 출력 묶음보다 먼저(D). 상태 `READY` | 사용자·Claude |
