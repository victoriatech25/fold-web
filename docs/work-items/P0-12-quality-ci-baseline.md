...
로컬 워크플로 구현과 비파괴 실패 시험은 완료했고, 2026-07-19 사용자가 `D0-12-A~H` 전체를 승인했다. GitHub CLI 설치·인증도 완료됐으며 GitHub Actions 실제 실행과 `main` 보호 규칙 검증을 진행한다.
...
- Git 전송 방식: HTTPS
- `main` branch protection 조회: 미설정(`404 Branch not protected`)

승인 전에는 로컬 workflow 작성과 저장소 메타데이터 읽기까지만 수행했다. 승인과 GitHub CLI 인증 완료 후 별도 branch·pull request에서 실제 workflow를 검증한다.
...
1. 현재 변경을 별도 branch에 commit·push한다.
2. pull request에서 Actions workflow 전체를 실제 실행한다.
3. `Quality and PostgreSQL integration` 성공을 확인한다.
4. 관리자 권한으로 `main` branch rule을 적용한다.
5. 필수 check를 의도적으로 실패시킨 PR이 병합 차단되는지 확인하고 복원한다.
6. P0-12와 P0 전체 상태를 `DONE`으로 변경한다.
