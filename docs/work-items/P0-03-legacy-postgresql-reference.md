...
| `sellplaninfo` | 40 | 절곡 DXF 원문(`dxfdata`). 재단 자료가 아니다 |
...
> **기록 보정(2026-08-30).** `sellplaninfo` 를 처음에 "재단/DXF 관련 표본" 으로 적었으나 이름을 보고 넘겨짚은 것이었다. `P2-B03` 에서 40건을 직접 열어 보니 내용은 **절곡 DXF 원문**(`dxfdata`, 38만 바이트)이다. **레거시는 재단 결과를 DB 에 되돌려 쓰지 않는다.** 화면과 출력물로만 썼다. 자세한 것은 [P2-B03 상세계획](./P2-B03-cutting-contract.md) 참조.
