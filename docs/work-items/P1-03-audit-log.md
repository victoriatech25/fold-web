...
| `02` | `READY` | D1-03-A~L 결정 승인 | 사용자 승인 |
| `03` | `TODO` | Prisma v2와 append-only migration | 빈 DB·upgrade·권한 시험 |
| `04` | `TODO` | typed action catalog·payload guard·writer | 단위 시험 |
| `05` | `TODO` | 기존 auth·admin writer 전환 | transaction·secret 비노출 |
| `06` | `TODO` | 조직 범위 repository·service·API | permission·IDOR·cursor |
| `07` | `TODO` | 감사 관리자 UI·navigation | 접근성·반응형 |
| `08` | `TODO` | PostgreSQL 통합·migration 회귀 | update/delete 거부 |
| `09` | `TODO` | Playwright·CI | 관리자·일반 사용자 흐름 |
| `10` | `TODO` | 문서·Chrome·Edge 사용자 검수 | 완료 대조 |
...
- [ ] 기존 audit 행을 손실 없이 v2 schema로 upgrade한다.
- [ ] application role과 Prisma에서 audit UPDATE·DELETE가 모두 거부된다.
- [ ] 성공 mutation과 audit이 함께 commit 또는 rollback된다.
- [ ] action·before·after·metadata에 password·token·secret이 들어갈 수 없다.
- [ ] 사건 당시 actor와 대상·이전/이후·시각·request ID를 조회할 수 있다.
- [ ] 타 조직 event와 `audit.read` 없는 사용자는 목록·상세에 접근할 수 없다.
- [ ] system role·reserved audit permission을 custom role로 우회할 수 없다.
- [ ] v1과 v2 event를 한국어 UI에서 안전하게 표시한다.
- [ ] 단위·migration·PostgreSQL·Playwright·필수 CI가 통과한다.
- [ ] Chrome·Edge 사용자 직접 검수가 승인된다.

## 16. 사용자 결정 게이트

`D1-03-A~L` 승인 전에는 Prisma migration, permission seed, writer와 관리자 UI 구현을 시작하지 않는다. 일부 권장안을 변경하면 모델·보안·조회 계약과 테스트 영향을 먼저 수정한 뒤 착수한다.
