...
> 기준일: 2026-07-19
>
> 기준 브랜치/PR: `codex/p1-rbac` / PR 준비 중
>
...
| 백엔드/DB/인증 | P1-02 검수 중 | PostgreSQL·Prisma 기반 독자 인증, DB session, 조직 membership, RBAC와 관리자 UI 구현 |
...
1. P1-02 관리자 UI를 Chrome·Edge에서 사용자 직접 검수하고 승인 기록을 남긴다.
2. P1-03에서 관리·인증·출력의 append-only 감사 조회와 전후 비교 형식을 완성한다.
3. P1-04에서 실제 전송 없이 기계 연동 `planned` 항목과 읽기 계약을 노출한다.
4. P1-05~07에서 절곡 필드 계약과 Prisma 기반 도면 저장·자동 저장·복원을 연결한다.
5. 계산 방식, V-CUT 적용 여부, 선별 계산 제외를 UI에 노출하고 설정별 회귀 테스트를 추가한다.
6. Playwright로 핵심 편집 흐름과 2D/3D/전개도 선택 동기화를 자동화한다.
7. 박스 기준선을 사용자가 직접 지정할 수 있게 하고 복잡 형상 검증을 추가한다.
8. 합의된 MFC 기준 샘플로 전개 폭과 절곡 반경 치수를 대조한다.
9. 제작 출력은 P1-14~16의 geometry·DXF 순서로 구현한다.
