...
import { previewSheetItem } from "@/server/sheet-items/sheet-item-service";
...
export async function POST(request: Request, context: Context) {
  ...
  try {
    ...
    const parsed = sheetItemFieldsSchema.safeParse(body.value);
    if (!parsed.success) return apiErrorResponse(requestId, 400, "INVALID_REQUEST", "원판 품목 입력값을 확인해 주세요.");
    return jsonResponse({ data: await previewSheetItem(getPrisma(), auth.context, { ...parsed.data, ...ids }) }, requestId);
  } catch (error) { return materialRouteErrorResponse(error, requestId, "preview sheet item"); }
}
