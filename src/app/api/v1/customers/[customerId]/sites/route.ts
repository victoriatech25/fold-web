...
import { createCustomerSite } from "@/server/customers/customer-service";
...
const createSchema = z.strictObject(customerSiteFieldsSchema);
...
export async function POST(request: Request, context: Context): Promise<Response> {
  ...
  try {
    ...
    if (!parsed.success) return apiErrorResponse(requestId, 400, "INVALID_REQUEST", "고객 현장 정보를 확인해 주세요.");
    const data = await createCustomerSite(getPrisma(), authorization.context, {
      ...parsed.data,
      customerId,
      requestId,
    });
    ...
    return customerRouteErrorResponse(error, requestId, "create customer site");
  }
}
