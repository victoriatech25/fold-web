...
import { removeOrderFoldItem } from "@/server/orders/order-fold-service";
...
export async function POST(request: Request, context: { params: Promise<{ orderId: string; itemId: string }> }) {
  ...
  try {
    ...
    if (!parsed.success || parsed.data.expectedItemLockVersion === undefined) return apiErrorResponse(requestId, 400, "INVALID_REQUEST", "절곡 작업 제거 정보를 확인해 주세요.");
    return jsonResponse({ data: await removeOrderFoldItem(getPrisma(), auth.context, { orderId, itemId, expectedOrderLockVersion: parsed.data.expectedOrderLockVersion, expectedItemLockVersion: parsed.data.expectedItemLockVersion, requestId }) }, requestId);
  } catch (error) {
