...
import { orderTransitionSchema } from "@/server/orders/order-transition-schema";
...
export async function POST(request: Request, context: { params: Promise<{ orderId: string }> }) {
  ...
  try {
    const auth = await authorizeOrderRequest(request, requestId, "order.approve", true);
    ...
    const parsed = orderTransitionSchema.safeParse(body.value);
    if (!parsed.success) return apiErrorResponse(requestId, 400, "INVALID_REQUEST", "수주 상태 전이 요청을 확인해 주세요.");
    return jsonResponse({ data: await transitionOrder(getPrisma(), auth.context, { orderId, ...parsed.data, requestId }) }, requestId);
  } catch (error) {
