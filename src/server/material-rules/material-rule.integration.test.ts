...
integration.sequential("material calculation rule integration", () => {
  ...
  beforeAll(async () => {
    ...
    context = { sessionId: crypto.randomUUID(), userId: user.id, displayName: user.displayName, membershipId: crypto.randomUUID(), departmentId: null, organizationId: organization.id, organizationCode: organization.code, organizationName: organization.name, platformAdmin: false, roleKeys: ["APPROVER"], permissions: ["material.read", "material.write", "material.approve"], expiresAt: new Date("2027-01-01T00:00:00Z") };
