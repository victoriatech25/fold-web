...
export async function authorizeMaterialRequest(request: Request, requestId: string, permission: PermissionKey, mutation: boolean): Promise<ApiAuthorizationResult> {
...
export function materialRouteErrorResponse(error: unknown, requestId: string, operation: string): Response {
  ...
  console.error("Material request failed.", { requestId, operation, errorName: error instanceof Error ? error.name : "UnknownError" });
  return apiErrorResponse(requestId, 500, "INTERNAL_ERROR", "서버에서 재질 요청을 처리하지 못했습니다.");
}
